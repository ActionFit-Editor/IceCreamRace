...
- 서버 시간이나 DevTool 시간이 필요하면 `ActionFit.Time.IClock`, calendar와 24시간 미만의 `calendarDayBoundaryOffset`을 함께 주입합니다. Clock source와 UTC/디바이스 로컬 calendar 선택은 독립적이며, 양수 offset은 해당 달력의 논리 자정을 늦춥니다. 기존 생성자는 offset 0을 유지하고 active legacy 숫자 tick에는 offset을 적용하지 않습니다.
